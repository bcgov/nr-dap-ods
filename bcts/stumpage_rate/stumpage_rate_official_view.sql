-- For dynamic M Query Parameters
CREATE OR REPLACE VIEW bcts_staging.vw_stumpage_rate_official_view AS
SELECT
    data.*,
    CASE
...
FROM (
    ...
    WHERE 1 = 1
        -- ts.auction_date BETWEEN DATE '2025-04-01'  -- beginning of current fiscal (Moved to Power BI M Query parameters)
...
GRANT SELECT ON bcts_staging.vw_stumpage_rate_official_view TO BCTS_DEV_ROLE;
GRANT SELECT ON bcts_staging.vw_stumpage_rate_official_view TO proxy_bcts_bi;

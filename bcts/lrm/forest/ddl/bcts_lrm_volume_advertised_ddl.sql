...
CREATE VIEW bcts_staging.forestview_v_cp AS
SELECT * FROM lrm_replication.v_cp;

CREATE MATERIALIZED VIEW BCTS_STAGING.MV_VOLUME_ADVERTISED_LRM AS
WITH LV AS
(
        SELECT
            B.LICN_SEQ_NBR,
...
AUC AS
    (
        ...
        FROM
            BCTS_STAGING.FORESTVIEW_V_LICENCE_ACTIVITY_ALL A
        WHERE
...
FROM
    BCTS_STAGING.FORESTVIEW_V_LICENCE L
    LEFT JOIN BCTS_STAGING.FORESTVIEW_V_CP CP

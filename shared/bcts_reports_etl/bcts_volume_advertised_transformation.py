...
def refresh_mat_views():
    ...
    sql_statement = \
    """
    REFRESH MAERIALIZED VIEW BCTS_STAGING.MV_ADVERTISED_LRM;
...
def publish_datasets():
    ...
    CREATE TABLE BCTS_STAGING.VOLUME_ADVERTISED_OFFICIAL
    AS SELECT * 
    FROM BCTS_STAGING.VOLUME_ADVERTISED_OFFICIAL_HIST
    ...
    DROP TABLE IF EXISTS BCTS_REPORTING.VOLUME_ADVERTISED_OFFICIAL;
    CREATE TABLE BCTS_REPORTING.VOLUME_ADVERTISED_OFFICIAL
    AS SELECT * 
    FROM BCTS_STAGING.VOLUME_ADVERTISED_OFFICIAL;


    DROP TABLE IF EXISTS BCTS_REPORTING.VOLUME_ADVERTISED_LRM;
    CREATE TABLE BCTS_REPORTING.VOLUME_ADVERTISED_LRM
    AS SELECT * 
    FROM BCTS_STAGING.MV_VOLUME_ADVERTISED_LRM;

    DROP TABLE IF EXISTS BCTS_STAGING.VOLUME_ADVERTISED_MAIN;
    CREATE TABLE BCTS_STAGING.VOLUME_ADVERTISED_MAIN
    AS SELECT * 
    FROM BCTS_STAGING.MV_VOLUME_ADVERTISED_MAIN_HIST
    WHERE report_frequency = 'Fiscal Year to Date'
        AND report_end_date = (
        SELECT MAX(report_end_date)
        FROM BCTS_STAGING.MV_VOLUME_ADVERTISED_MAIN_HIST

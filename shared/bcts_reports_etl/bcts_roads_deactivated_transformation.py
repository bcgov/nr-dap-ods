...
from transformation_queries.roads_deactivated import get_roads_deactivated_query
...
def run_report(connection, cursor, start_date, end_date):
    ...
    sql_statement = get_roads_deactivated_query(start_date, end_date)
...
if __name__ == "__main__":
    ...
    if max_report_exist_date is not None:
        if max_report_exist_date == end_date:
            logging.info("BCTS roads deactivated report is already up-to-date! ")
            ...
            # Run each report
            logging.info(f"Running BCTS roads deactivated report for the reporting start_date {start_date} and end date {end_date}...")

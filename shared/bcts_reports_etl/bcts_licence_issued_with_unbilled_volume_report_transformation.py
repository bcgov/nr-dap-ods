...
def publish_datasets():
    ...
    INSERT INTO bcts_staging.licence_issued_with_unbilled_volume_hist SELECT * FROM bcts_staging.licence_issued_with_unbilled_volume_main;
...
def licence_issued_with_unbilled_volume_report_exists(report_date):
    ...
    try:
        ...
        if e.pgcode == errorcodes.UNDEFINED_TABLE:
            logging.warning("Table bcts_reporting.licence_issued_with_unbilled_volume_main does not exist.")
            return False
        else:
            logging.error(f"Error executing the SQL script: {e}")
...
if __name__ == "__main__":
    ...
    if licence_issued_with_unbilled_volume_report_exists(report_date):
        ...
        logging.info("Deleting existing entries in the licence_issued_with_unbilled_volume_hist table for the report month if any...")
        truncate_licence_issued_with_unbilled_volume_hist(report_date)
        logging.info("Updating licence issued with unbilled volume datasets to the reporting layer...")
        publish_datasets()
        logging.info("Licence issued with unbilled volume datasets in the reporting layer have been updated!")

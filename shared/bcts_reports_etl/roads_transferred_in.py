def get_roads_transferred_in_query(start_date, end_date):
    sql_statement = \
    f"""    
    INSERT INTO bcts_staging.roads_transferred_in_hist(
    business_area_region_category, business_area_region, business_area, business_area_code, road_seq_nbr, uri, road_road_name, rcls_accounting_type, rdst_steward_name, poc, pot, length, transfer_date, const_method_type, fiscal_year_start_date, report_end_date   
    )
    SELECT DISTINCT
    case
        when

...
def extract_from_oracle(username, password, host, port, database, sql_query):
    ...
    print(dsn)
    connection_pool = oracledb.SessionPool(user=username, password=password, dsn=dsn, encoding="UTF-8")
...
try:
    rows, column_names = extract_from_oracle(username, password, host, port, database, sql_query)

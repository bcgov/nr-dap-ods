SELECT
	...
	CASE
		WHEN SDO_CONTAINS(
			HAG.GEOMETRY,
			SDO_GEOM.SDO_CENTROID(HAG.GEOMETRY)
		) = 'TRUE' THEN (
			SDO_CS.TRANSFORM(SDO_GEOM.SDO_CENTROID(HAG.GEOMETRY), 4326)
		) .SDO_POINT.Y
		ELSE (
			SDO_CS.TRANSFORM(SDO_GEOM.SDO_POINTONSURFACE(HAG.GEOMETRY), 4326)
		) .SDO_POINT.Y
	END AS LAT,
	CASE
		WHEN SDO_CONTAINS(
...
SELECT
	...
	CASE
		WHEN SDO_CONTAINS(
			RSG.GEOMETRY,
			SDO_GEOM.SDO_CENTROID(RSG.GEOMETRY)
		) = 'TRUE' THEN (
			SDO_CS.TRANSFORM(SDO_GEOM.SDO_CENTROID(RSG.GEOMETRY), 4326)
		) .SDO_POINT.Y
		ELSE (
			SDO_CS.TRANSFORM(SDO_GEOM.SDO_POINTONSURFACE(RSG.GEOMETRY), 4326)
		) .SDO_POINT.Y
	END AS LAT,
	CASE
		WHEN SDO_CONTAINS(
	...
	CASE
		WHEN OU.ORGANIZATION_UNIT_NAME IN (
			'North Island - Central Coast Natural Resource District',
			'South Island Natural Resource District',
			'Campbell River Natural Resource District',
			'Haida Gwaii Natural Resource District'
	...
	END AS NR_REGION
FROM
	APP_RRS.RESOURCE_ROAD_TENURE t

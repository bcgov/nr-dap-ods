...
FROM
	(
		...
		FROM
			(
				...
				FROM
					(
						...
						FROM
							rrs_replication.x_nrs_consolidated_vw AS nrs_vw -- join with x_nrs_consolidated_vw
			...
			LEFT JOIN geometry.natural_resource_boundaries AS nr_boundaries -- determines which shapes are contained in which NR boundaries
			ON ST_Contains(nr_boundaries.geom, with_lat_lon.geometry)
	) AS with_nr_boundaries -- matches ATS free text to BC place names
	LEFT JOIN generalized_geographic_places AS geo_names ON with_nr_boundaries.location ILIKE geo_names.geographical_name;
